    ...
    registerEventListeners() {
        ...
        if (this.canShowFilePicker) {
            this.form.querySelector('.' + this.CSS.IMAGEBROWSER).addEventListener('click', (e) => {
                e.preventDefault();
                let options = this.editor.moodleOptions.filepicker.image;
                options.formcallback = (params) => {
...
export default new Promise(async(resolve) => {
    ...
    const toolbarImageLabel = await getString('toolbarimage', 'tiny_media');

    tinyMCE.PluginManager.add('tiny_media/plugin', (editor) => {
        const mediaImage = new MediaImage(editor);
        editor.ui.registry.addToggleButton('tiny_media/image', {
            icon: 'image',
            text: toolbarImageLabel,
            onAction: () => {
        ...
        editor.ui.registry.addContextToolbar('tiny_media/imageCtx', {
            predicate: node => {
                return node.nodeName.toLowerCase() === 'img';
            },
            items: 'tiny_media/image',
            position: 'node',
            scope: 'node'
        });
